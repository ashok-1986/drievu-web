...
export default function HomePage() {
  ...
  return (
    <div className="flex flex-col w-full">
      ...
      <div className="w-full h-24 md:h-36 bg-gradient-to-b from-brand-slate via-brand-slate/60 to-brand-paper pointer-events-none select-none" />
        ...
        <section className="py-24 px-6 max-w-[1200px] mx-auto w-full relative z-30">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            <div className="lg:col-span-5 space-y-6">
              ...
              <div className="bg-brand-mist p-5 rounded-xl border-l-4 border-brand-teal font-body text-xs text-brand-grey  font-mono">
            ...
            <div className="lg:col-span-7 grid grid-cols-1 sm:grid-cols-3 gap-6">
              <TactileLink href="/track-record" variant="ghost" className="bg-brand-mist p-8 rounded-2xl border border-brand-grey/15 flex flex-col justify-center text-center sm:text-left hover:border-brand-teal/40 transition-colors duration-200 ease-[cubic-bezier(0.16,1,0.3,1)] active:scale-[0.97]">
                <span className="font-display font-normal text-5xl text-brand-slate mb-2 block">18+</span>
                <span className="font-display font-normal text-sm text-brand-slate block mb-1">Landmark Projects</span>
                <span className="font-body font-normal text-xs text-brand-grey">Stadiums, metros, airports & infrastructure.</span>
              </TactileLink>
              
              <TactileLink href="/track-record" variant="ghost" className="bg-brand-mist p-8 rounded-2xl border border-brand-grey/15 flex flex-col justify-center text-center sm:text-left hover:border-brand-teal/40 transition-colors duration-200 ease-[cubic-bezier(0.16,1,0.3,1)] active:scale-[0.97]">
                <span className="font-display font-normal text-5xl text-brand-slate mb-2 block">20+</span>
                <span className="font-display font-normal text-sm text-brand-slate block mb-1">Years Track Record</span>
                <span className="font-body font-normal text-xs text-brand-grey">Executive engineering leadership.</span>
              </TactileLink>

              <TactileLink href="/track-record" variant="ghost" className="bg-brand-mist p-8 rounded-2xl border border-brand-grey/15 flex flex-col justify-center text-center sm:text-left hover:border-brand-teal/40 transition-colors duration-200 ease-[cubic-bezier(0.16,1,0.3,1)] active:scale-[0.97]">
                <span className="font-display font-normal text-5xl text-brand-teal mb-2 block">4</span>
                <span className="font-display font-normal text-sm text-brand-slate block mb-1">Core Disciplines</span>
      ...
      <section className="bg-brand-slate text-brand-paper py-24 px-6 relative z-30 overflow-hidden">
        <div className="max-w-[1200px] mx-auto relative z-10">
          <div className="text-center max-w-2xl mx-auto mb-20">
            <h2 className="font-display font-normal text-3xl md:text-5xl text-white  mb-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-8 relative z-10">
            {workProcess.map((step, index) => (
              <ScrollReveal key={step.name} delay={index * 0.08} direction="up">
                <TactileLink href="/consultation" variant="ghost" className="flex flex-col items-start lg:items-center lg:text-center group bg-transparent hover:bg-white/5 rounded-2xl p-4">
                  <div className="w-14 h-14 rounded-full bg-brand-slate border-2 border-brand-teal text-white font-display font-normal text-base flex items-center justify-center mb-6">
                    {step.step}
                  </div>
                  <h3 className="font-display font-normal text-lg text-white mb-2">
                    {step.name}
                  </h3>
                  <p className="font-body font-normal text-xs text-brand-paper/70 ">
                    {step.desc}
                  </p>
                </TactileLink>
              </ScrollReveal>
